...
class Temppanel(ttk.Frame):
    ...
    def __init__(self, master, name, extId, gcode):
        ...
        self.cv.create_rectangle(1, 15, 0, 35, 
    ...
    def setTemp(self, val, obj):  
        try:
            ...
            self.setSP(val)
            if self.extId:
                model.xmtQ.put(self.extId)  # select extruder (if any)
            model.xmtQ.put('{} S{}'.format(self.gcode, float(self.setPoint)))
        finally:
            self.shutdown(obj)
